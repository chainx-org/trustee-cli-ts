...
class Api {
    ...
    public async getTrusteeSessionInfo(session_num: number): Promise<TrusteeSessionInfo> {
        await this.ready()
        // @ts-ignore
        const sessionInfo = await this.api.rpc.xgatewaycommon.bitcoinTrusteeSessionInfo(session_num);
